...
int main(int argc, char** argv){
    ...
    Jugador* model = new Jugador("models/hombre.obj", translate, camera);
    ...
    while (running) {
        ...
        if (renderiza) {
            ...
            if (checkCollition) { // Bandera para buscar colisiones sobre Camara/Modelo
                Model* colision = OGLobj->lookForCollition(true);
                if (OGLobj->lookForCollition(true) != NULL) { // Llamamos a la funcion de colision 
                    const glm::vec3& posicion = *OGLobj->getMainModel()->getNextTranslate();
                    if (colision->name.compare("casita") == 0 &&
                        posicion.x >= 0 && posicion.x <= -10
                        && posicion.y >= 19 && posicion.y <= 29
                        && posicion.z >= -55 && posicion.z <= -60) 
                    {
                        model->setNextTranslate(model->getNextTranslate());
            ...
            if (escena != OGLobj) {
                ...
                if (OGLobj->lookForCollition(true) != NULL) { // Llamamos a la funcion de colision   
                    Model* modeloColision = OGLobj->lookForCollition(true);
                    if (modeloColision != NULL) {
                        model->setNextTranslate(model->getTranslate());
                        model->setNextRotX(model->getRotX());
                        model->setNextRotY(model->getRotY());
                        model->setNextRotZ(model->getRotZ());
